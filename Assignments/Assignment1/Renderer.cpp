...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	// Create Nanogui Rotation Bar
	nanogui::ref<nanogui::Window> nanoguiWindow_Rotation = gui_1->addWindow(Eigen::Vector2i(200, 0), "Nanogui Rotation Bar");
	gui_1->addGroup("Bunny Rotation");
	gui_1->addVariable("a", a_degree)->setSpinnable(true);
	gui_1->addButton("Rotate by A degrees", [this]() {
		//std::cout << a_degree;
		glm::mat4 bunny_model_mat = m_animation->get_model_mat();

		glm::mat4 rotation_mat = glm::rotate(glm::mat4(1.0F), glm::radians(a_degree), glm::vec3(1.0F, 0.0F, 0.0F));

		m_animation->set_model_mat(rotation_mat * bunny_model_mat);
	});
